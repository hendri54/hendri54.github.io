...
startDate = datetime(year1, 8, 18);
endDate = datetime(year1, 12, 1);
...
% i1 = i1 + 1;
topicV{i1} = markdownLH.SubTopic({'Fall break'}, datetime(year1, 11, 25));
...
if showExample
   ...
   exampleStr = '[solution for example](olg/OLG_example.pdf)';
else
...
iTopic = iTopic + 1;
topicListV{iTopic} = markdownLH.Topic('Overlapping Generations', topicV(1 : i1), classDateV);
...
iTopic = iTopic + 1;
topicListV{iTopic} = markdownLH.Topic('Infinite Horizon, Discrete Time Models', topicV(1 : i1), classDateV);
...
iTopic = iTopic + 1;
topicListV{iTopic} = markdownLH.Topic('Endogenous Growth', topicV(1 : i1), classDateV);
...
if false
   ...
   iTopic = iTopic + 1;
   topicListV{iTopic} = markdownLH.Topic('Search and Matching', topicV(1 : i1), classDateV);
end
